...
function waitForHandshakeFailure(socket) {
    return new Promise((resolve) => {
        socket.once("error", (error) => resolve(error instanceof Error ? error.message : String(error)));
    });
}
function waitForMessageTypes(socket, expectedTypes) {
    return new Promise((resolve, reject) => {
        const messages = [];
...
describe("createServer", () => {
    ...
    test("dispatches and cancels a running job over websocket", async () => {
        ...
        const workspace = join(root, "workspace");
        mkdirSync(workspace);
        const fakeOpencode = join("/Users/baominh/Dev/Personal/Automation/goclaw/.worktrees/opencode-worker-server/workers/opencode-node/src/test/fixtures", "fake-opencode.js");
        const server = createServer({
            port: 0,
            path: "/ws",
            authToken: "secret-token",
